...
Public Class Host
    ...
    Public Shared Function GetNewInspectorItem(<CallerMemberName> Optional memberName As String = "", <CallerFilePath> Optional fileName As String = "", <CallerLineNumber> Optional lineNumber As Integer = 0) As InspectorItem
        ...
        If GlobalSettings.Settings.InspectorEnabled And Not GlobalSettings.Settings.ExcelMode And Not GlobalSettings.Settings.CAPEOPENMode Then
            Return New Inspector.InspectorItem With {.CodePath = (fileName & "#L" & lineNumber).Replace(fileName.Substring(0, fileName.IndexOf("\dwsim_AlexanderSemenyak\") + 7), "https://github.com/DanWBR/dwsim6/blob/windows").Replace("\", "/")}
            Return New Inspector.InspectorItem With {.CodePath = (fileName & "#L" & lineNumber).Replace(fileName.Substring(0, fileName.LastIndexOf("\dwsim_AlexanderSemenyak\") + 7), "https://github.com/AlexanderSemenyak/dwsim5/tree/windows").Replace("\", "/")}
        Else
            Return Nothing
        End If
    ...
    Public Shared Sub CheckAndAdd(ii As InspectorItem, callingmethod As String, method As String, name As String, description As String, Optional current As Boolean = False)
        ...
        If ii IsNot Nothing Then
            ...
            If Host.CurrentItem IsNot Nothing Then
                ...
                Host.CurrentItem.Paragraphs.Add(String.Format("<div style='color:gray'>[Вызов функции <i>{0}</i> ({1} - {2})]</div>", method, name, description))
...
Public Class InspectorItem
    ...
    Public Function GetHTML() As String
        ...
        stb.AppendLine("<hr>")
        'stb.AppendLine(String.Format("<div><div style='float:right;height:40px;line-height:40px;vertical-align:middle;'><a target='_blank' style='border:0;' href='{0}'><img style='border:0;' src='{2}' alt='Просмотреть на GitHub' width='200'></a></div><div style='float:left;height:40px;line-height:40px;vertical-align:middle;'><b>Source Code</b>: {1}</div></div>", CodePath, CodePath.Replace("https://github.com/AlexanderSemenyak/dwsim5/tree/windows/", ""), GetImagePath("viewongithub.png")))
